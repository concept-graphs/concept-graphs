...
# add batch size setting to reduce GPU memory usage
def compute_clip_features_batch_size(image, detections, clip_model, clip_preprocess, clip_tokenizer, classes, device, batch_size=8):
    # if no detections, return empty lists
    if len(detections.xyxy) == 0:
        return [], np.array([]), np.array([])
    ...
    # use small batch size to reduce GPU memory usage
    all_image_features = []
    for i in range(0, len(preprocessed_images), batch_size):
        batch = torch.cat(preprocessed_images[i:i+batch_size], dim=0).to(device)
        text_tokens_batch = clip_tokenizer(text_tokens[i:i+batch_size]).to(device)
    ...
    # 如果需要处理文本特征，可以在此启用文本推理
    # text_features = clip_model.encode_text(text_tokens_batch)
    # text_features /= text_features.norm(dim=-1, keepdim=True)

    # if you don't need text features, return empty text features
    text_feats = []  
